...
export const ArticleShareButtons: React.FC<ArticleShareButtonsProps> = ({ article, translations }) => {
    const siteUrl = window.location.href;
    const shareText = encodeURIComponent(
        `${article.title} - ${translations.siteTitle}`
    );
            ...
            <div className="flex items-center gap-2">
                {socialButtons.map(social => (
                    <a
                        ...
                        aria-label={`Share ${article.title} on ${social.name}`}
